...
typedef hpx::components::component<tbsla::hpx_::server::MatrixELL> MatrixELL_server_type;
HPX_REGISTER_COMPONENT(MatrixELL_server_type, MatrixELL_server);

typedef tbsla::hpx_::server::MatrixELL::get_data_action get_MatrixELL_data_action;
HPX_REGISTER_ACTION(get_MatrixELL_data_action);

std::size_t tbsla::hpx_::MatrixELL::get_n_col() {
  if(this->tiles.size() == 0)
    return 0;
  return tiles[0].get_data().get().get_n_col();
}

std::size_t tbsla::hpx_::MatrixELL::get_n_row() {
  if(this->tiles.size() == 0)
    return 0;
...
void tbsla::hpx_::MatrixELL::fill_cdiag(std::vector<hpx::id_type> localities, std::size_t nr, std::size_t nc, std::size_t cdiag, std::size_t gr, std::size_t gc) {
  ...
  for (std::size_t pr = 0; pr < gr; pr++) {
    for (std::size_t pc = 0; pc < gc; pc++) {
      this->tiles[pr * gc + pc] = tbsla::hpx_::client::MatrixELL(this->localities[(pr * gc + pc) * nl / nt], nr, nc, cdiag, pr, pc, gr, gc);
    }
  }
}
...
void tbsla::hpx_::MatrixELL::fill_cqmat(std::vector<hpx::id_type> localities, std::size_t nr, std::size_t nc, std::size_t c, double q, unsigned int seed, std::size_t gr, std::size_t gc) {
  ...
  for (std::size_t pr = 0; pr < gr; pr++) {
    for (std::size_t pc = 0; pc < gc; pc++) {
      this->tiles[pr * gc + pc] = tbsla::hpx_::client::MatrixELL(this->localities[(pr * gc + pc) * nl / nt], nr, nc, c, q, seed, pr, pc, gr, gc);
    }
  }
}

static tbsla::hpx_::client::Vector tbsla::hpx_::detail::spmv_part(tbsla::hpx_::client::MatrixELL const& A_p, tbsla::hpx_::client::Vector const& v_p, tbsla::hpx_::client::Vector const& r_p) {
  using hpx::dataflow;
  using hpx::util::unwrapping;

  hpx::shared_future<tbsla::hpx_::detail::MatrixELL> A_data = A_p.get_data();
  hpx::shared_future<tbsla::hpx_::detail::Vector> v_data = v_p.get_data();
  return dataflow(hpx::launch::async,
    unwrapping([r_p](tbsla::hpx_::detail::MatrixELL const& A, tbsla::hpx_::detail::Vector const& v) -> tbsla::hpx_::client::Vector {
...
void tbsla::hpx_::MatrixELL::wait() {
  for (std::size_t i = 0; i < this->tiles.size(); ++i) {
